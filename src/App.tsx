...
import { DatabaseInspectorModal } from '@/components/auth/DatabaseInspectorModal';
...
function Shell({ children }: { children: ReactNode }) {
  ...
  const { setIsDbModalOpen } = useAuth();
    ...
    <div className="noise min-h-[100dvh] bg-[hsl(var(--background))]">
      <aside className={`fixed inset-y-0 left-0 z-40 flex w-[242px] flex-col bg-[hsl(var(--sidebar))] px-5 py-6 text-[hsl(var(--sidebar-foreground))] transition-transform duration-200 md:translate-x-0 ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="mt-auto">
          ...
          <div className="border-t border-[hsl(var(--sidebar-foreground)/.1)] pt-3">
        ...
        <header className="sticky top-0 z-20 flex h-[70px] items-center justify-between border-b border-[hsl(var(--border)/.8)] bg-[hsl(var(--background)/.9)] px-5 backdrop-blur-md md:px-9">
          ...
          <div className="flex items-center gap-2.5">
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-lg border border-[hsl(var(--border))] bg-[hsl(var(--card))] px-2.5 py-1.5 text-[11px] font-medium text-[hsl(var(--muted-foreground))] hover:border-[hsl(var(--primary)/.4)] hover:text-[hsl(var(--foreground))] transition-colors shadow-2xs"
              data-testid="button-header-assess-database"
            >
              <Database className="h-3.5 w-3.5 text-[hsl(var(--primary))]" />
              <span className="hidden sm:inline">Assess Database</span>
            </button>
            <button onClick={() => toast({ title: 'You are all caught up', description: 'No new branch messages.' })} className="relative rounded-lg p-2 text-[hsl(var(--muted-foreground))] hover:bg-[hsl(var(--muted))] hover:text-[hsl(var(--foreground))]" aria-label="View notifications" data-testid="button-notifications"><Bell className="h-[18px] w-[18px]" /><span className="absolute right-1.5 top-1.5 h-1.5 w-1.5 rounded-full bg-[hsl(var(--accent))]" /></button>
